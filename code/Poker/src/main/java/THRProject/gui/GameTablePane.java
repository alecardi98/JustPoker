 ...
 * 
 * MODIFICHE:
 * - Aggiunto input per puntata con TextField
 * - Aggiunta selezione carte per cambio
 * - Migliorata visualizzazione carte coperte/scoperte
 * - Aggiunta visualizzazione piatto e fiches
 * - Aggiunti pulsanti Quit e Pronto
...
public class GameTablePane extends VBox {
    ...
    public GameTablePane(Client client, Game game, int clientId) {
        ...
        cardsPane.setStyle("-fx-border-color: white; -fx-border-width: 2px; -fx-padding: 15px; -fx-background-color: rgba(255,255,255,0.1);");
        ...
        betAmountField.setText("10"); // Valore di default
        ...
        foldBtn.setOnAction(e -> client.invioLascia());
        ...
        getChildren().addAll(infoPane, opponentsPane, cardsPane, betPane, buttonsPane, endButtonsPane);
    }
    ...
    public void refresh() {
        ...
        boolean myTurn = game.getCurrentTurn() == clientId && me != null && !me.getStatus().isFold() && !me.getStatus().isEnd();
        ...
        betAmountField.setDisable(!myTurn || (game.getPhase() != GamePhase.APERTURA && game.getPhase() != GamePhase.PUNTATA));
    }
    ...
    private VBox createCardView(Card card) {
        ...
        box.setOnMouseClicked(e -> {
            Game game = client.getGameView();
            if (game != null && game.getPhase() == GamePhase.ACCOMODO && game.getCurrentTurn() == clientId) {
                if (selectedCards.contains(card)) {
                    selectedCards.remove(card);
                    box.setStyle("-fx-border-color: black; -fx-padding: 5px; -fx-background-color: white; -fx-cursor: hand;");
                } else if (selectedCards.size() < 3) {
                    selectedCards.add(card);
                    box.setStyle("-fx-border-color: red; -fx-border-width: 3px; -fx-padding: 5px; -fx-background-color: lightyellow; -fx-cursor: hand;");
                }
            }
        });
